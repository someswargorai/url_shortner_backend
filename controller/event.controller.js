...
const trackEventController = async (req, res) => {
  try {
    ...
    if (!projectApiKey) {
      return res
        .status(401)
        .json({ success: false, message: "Missing Project API Key" });
    }
    ...
    if (!project) {
      return res
        .status(404)
        .json({ success: false, message: "Invalid Project API Key" });
    }
    ...
    if (notification && project.userId?.email) {
      ...
      const formattedTime =
        new Date(newEvent.timestamp).toLocaleString("en-US", {
          timeZone: "UTC",
          dateStyle: "medium",
          timeStyle: "medium",
        }) + " UTC";
      ...
      const emailText =
        `Event "${event}" triggered in project "${project.name}" at ${formattedTime}.\n\n` +
      ...
      const emailHtml = `
            ...
            <body>
              <div class="container">
                ...
                <div class="content">
                  <p class="intro-text">
                    An event of type <strong>${event}</strong> was captured from your project <strong>${
        project.name
      }</strong>.
                  </p>
                  ...
                  <div class="btn-container">
                    <a href="${process.env.frontend_url ||
                      "http://localhost:3000"}/dashboard" class="action-btn" target="_blank" style="color: #ffffff !important;">View Live Logs</a>
                  </div>
                </div>
                <div class="footer">
...
const getProjectAiChatController = async (req, res) => {
  try {
    ...
    const prompt = `You are an expert Data Analyst AI for a software project named "${
      project.name
    }".
...
Provide a helpful, analytical answer. Use markdown for formatting. If the answer cannot be determined from the context, state that clearly but offer any related insights if possible. Keep it concise.`;
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to process AI chat" });
  }
};
...
const getProjectUserJourneysController = async (req, res) => {
  try {
    ...
    const events = await Event.find({
      projectId: project._id,
      $or: [{ userId: { $in: userKeys } }, { anonymousId: { $in: userKeys } }],
    }).sort({ timestamp: 1 });
    ...
    const journeys = Object.values(userGroups).map((group) => {
      ...
      for (let i = 0; i < sortedEvents.length - 2; i++) {
        const diff1 =
          new Date(sortedEvents[i + 1].timestamp || 0).getTime() -
          new Date(sortedEvents[i].timestamp || 0).getTime();
        const diff2 =
          new Date(sortedEvents[i + 2].timestamp || 0).getTime() -
          new Date(sortedEvents[i + 1].timestamp || 0).getTime();
        if (
